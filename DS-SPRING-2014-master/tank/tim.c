...
IM_TimeBaseInitTypeDef TIM_TimeBaseStructure;
...
static void f3d_timer4_pins(void) {
  ...
  // Configure PD13,PD14,PD15 as an Inputs
  GPIO_StructInit(&GPIO_InitStructure);
...
static void f3d_timer2_pins(void) {
  ...
  // Configure PD4,PD6,PD7 as an Inputs
  GPIO_StructInit(&GPIO_InitStructure);
...
void timer2_init(void) {
  ...
  // Configure the PortD 12-15 and assign to timer2
  f3d_timer2_pins();	
  ...
  // Enable the timer
  TIM_Cmd(TIM2, ENABLE);
  f3d_frequencytimer2(10);
}

void f3d_frequencytimer4(uint32_t freq) {
...
void f3d_frequencytimer2(uint32_t freq) {
  uint32_t period, prescale;
  if (freq) {
